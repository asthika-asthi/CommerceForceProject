...
export class ProductService {
  ...
  static async create(data: Partial<Product>): Promise<Product> {
    const id = uuidv4();
    const { sku, name, description, category, base_price, image_url, is_active = 1 } = data;
    ...
    await db.query(`
      INSERT INTO products (id, sku, name, description, category, base_price, image_url, is_active)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `, [id, sku, name, description, category, base_price, image_url, is_active ? 1 : 0]);
  ...
  static async update(id: string, data: Partial<Product>): Promise<Product> {
    ...
    const fields = Object.keys(data).filter(key => 
      ['sku', 'name', 'description', 'category', 'base_price', 'image_url', 'is_active'].includes(key)
    );
    ...
    const values = fields.map(field => {
      ...
      if (field === 'is_active') return val ? 1 : 0;
      return val;
    });
